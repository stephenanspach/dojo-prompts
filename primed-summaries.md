...
Spawn a subagent (Task tool, `subagent_type: "general-purpose"`) with this prompt. The subagent inherits the current session's model — do not specify one.
...
The subagent inherits the current session's model — do not specify one.
